<script>
  ...
  import Mat from './lib/Mat.svelte'
  ...
  import { cardImg, isDefense } from './lib/util.js'
  import { playBattleFx } from './lib/battleFx.js'
  ...
  // affordance lookups — the pure legal-payload derivations live in store.js
  // ($affordances); these thin wrappers keep the existing call sites working and
  // re-run whenever `$legal` changes. Board-shape helpers (first empty zone) and
  // intent senders stay here since they read `you` / send moves.
  $: aff = $affordances
  function summonOptions(iid) { return aff.summonOptions(iid) }
  function attackTargets(iid) { return aff.attackTargets(iid) }
  function activateOptions(iid) { return aff.activateOptions(iid) }
  function canActivate(iid) { return aff.canActivate(iid) }
  function canSet(iid) { return aff.canSet(iid) }
  function canFlip(iid) { return aff.canFlip(iid) }
  function canGeminiSummon(iid) { return aff.canGeminiSummon(iid) }
  function canSpecialSummon(iid) { return aff.canSpecialSummon(iid) }
  function canUnionEquip(iid) { return aff.canUnionEquip(iid) }
  function canUnionUnequip(iid) { return aff.canUnionUnequip(iid) }
  function canChangePos(iid) { return aff.canChangePos(iid) }
  ...
  // The DOM-level bump/flash/impact lives in lib/battleFx.js; App keeps only the
  // floating-damage list it renders.
  ...
  onMount(() =>
    battleFx.subscribe((fx) => {
      ...
      else playBattleFx(fx, floatDamage)
    }),
  )
...
<style>
  ...
  /* The board mat, its slots and per-zone highlight styles moved to lib/Mat.svelte
     (they scope to that component's markup now). What remains here is board chrome
     that App still renders: the place-a-card banner, damage floaters, battle banner. */
