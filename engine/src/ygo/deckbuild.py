...
later means editing data, not branching code. The :class:`BanList` model and its
file IO now live in :mod:`ygo.banlist` (re-exported here for backwards
compatibility); this module keeps only the construction rules and validation.
"""
...
# Re-exported so ``from ygo.deckbuild import BanList, load_banlist, ...`` keeps
# working; the definitions themselves live in ygo.banlist (which pricing.py can
# import without pulling in this whole deck-construction module).
from .banlist import (  # noqa: F401
    MAX_COPIES,
    NO_RESTRICTIONS,
    USER_BANLIST_SUBDIR,
    BanList,
